#region Assembly FrooxEngine, Version=2025.8.22.607, Culture=neutral, PublicKeyToken=null
// C:\Program Files (x86)\Steam\steamapps\common\Resonite\FrooxEngine.dll
...
public class UIBuilder
{
    ...
    public IAssetProvider<Sprite> CheckSprite
    {
        get
        {
            if (_checkSprite == null)
            {
                ...
                {
                    StaticTexture2D target = Root.World.RootSlot.AttachTexture(OfficialAssets.Common.Icons.Check, getExisting: true, uncompressed: true, directLoad: false, evenNull: false, (TextureWrapMode)1);
    ...
    {
        return world.RootSlot.AttachTexture(OfficialAssets.Common.Particles.Disc, getExisting: true, uncompressed: true, directLoad: false, evenNull: false, (TextureWrapMode)1);
    }

    public static IAssetProvider<Sprite> GetCircleSprite(World world)
    {
        World world2 = world;
        return world2.GetSharedComponentOrCreate("BasicUI_Circle", delegate (SpriteProvider sprite)
        {
            sprite.Texture.Target = GetCircleTexture(world2);
    ...
    //
    // Summary:
    //     Creates a panel which will nest subsequent UI elements until a matching NestOut
    //     call is made.
    ...
    {
        colorX? tint = Style.ButtonColor;
        return Button(in text, in tint);
    }
    ...
    {
        colorX? tint = Style.ButtonColor;
        return Button(in text, sprite, null, in tint, in Style.ButtonSpriteColor);
    }
    ...
    {
        colorX? tint = Style.ButtonColor;
        return Button(in text, sprite, null, in tint, in spriteTint, buttonTextSplit, buttonTextSplitGap);
    }

    public Button Button(Uri icon, LocaleString text)
    {
        colorX? tint = Style.ButtonColor;
        return Button(in text, null, icon, in tint, in Style.ButtonSpriteColor);
    }
    ...
    {
        colorX? buttonTint = Style.ButtonColor;
        return Button(spriteUrl, in buttonTint);
    }
    ...
    public Button Button(Uri spriteUrl, in colorX? buttonTint, in colorX spriteTint)
    {
        ...
        return Button(in text, null, spriteUrl, in buttonTint, in spriteTint);
    }

    public Button Button(IAssetProvider<Sprite> sprite, in colorX? buttonTint, in colorX spriteTint)
    {
        LocaleString text = null;
        return Button(in text, sprite, null, in buttonTint, in spriteTint);
    }
    ...
    {
        LocaleString text = null;
        return Button(in text, null, spriteUrl, in buttonTint, in spriteTint).SetupAction(action, doublePressDelay);
    }
    ...
    public Button Button<T>(Uri spriteUrl, ButtonEventHandler<T> action, T argument, float doublePressDelay = 0f)
    {
        LocaleString text = null;
        colorX? tint = null;
        colorX spriteTint = colorX.White;
        Button button = Button(in text, null, spriteUrl, in tint, in spriteTint);
    ...
    {
        LocaleString text = null;
        Button button = Button(in text, null, spriteUrl, in buttonTint, in spriteTint);
        button.SetupAction(action, argument, doublePressDelay);
        return button;
    }
    ...
    {
        colorX spriteTint = colorX.White;
        return Button(in text, null, null, in tint, in spriteTint);
    }
    ...
    {
        colorX? tint = null;
        return ButtonRef(in text, in tint, callback, argument, doublePressDelay);
    }
    ...
    public ReferenceRadio<T> ReferenceRadio<T>(string label, SyncRef<T> refField, T target) where T : class, IWorldElement
    {
        ...
        Text(in text, bestFit: true, Alignment.MiddleLeft);
        NestOut();
        NestInto(footer);
    ...
    public RawImage RawImage(IAssetProvider<ITexture2D> texture, bool preserveAspect = false)
    {
        ...
        return RawImage(texture, in tint, preserveAspect);
    }
    ...
    {
        colorX tint = colorX.White;
        return Image(sprite, in tint, preserveAspect);
    }
    ...
    public Slider<T> Slider<T>(float height, T value, T min, T max, bool integers, out Image line, out Image fillLine, out Image handle)
    {
        ...
        float2 v = float2.One;
        float2 position = v * (0f - num);
        float2 v2 = float2.One;
        float2 size = v2 * height;
        rectTransform.SetFixedRect(new Rect(in position, in size));
    ...
    //
    // Summary:
    //     Creates a horizontal layout for UI elements. Will automatically nest subsequent
    //     UI elements until a matching NestOut call is made.
    //
    // Parameters:
    //   spacing:
    //
    ...
    //
    // Summary:
    //     Creates a vertical layout for UI elements. Will automatically nest subsequent
    //     UI elements until a matching NestOut call is made.
    //
    // Parameters:
    //   spacing:
    ...
    //
    // Summary:
    //     Creates a grid layout for UI elements. Will automatically nest subsequent UI
    //     elements until a matching NestOut call is made.
    public GridLayout GridLayout()
    {
        float2 v = float2.One;
        float2 cellSize = v * 64;
        float2 spacing = float2.Zero;
        return GridLayout(in cellSize, in spacing);
    }
    ...
    //
    // Summary:
    //     Creates a scroll area for UI elements. Will automatically nest subsequent UI
    //     elements until a matching NestOut call is made.
    //
    // Parameters:
    //   alignment:
    ...
    public static void SetupButtonColor(Button button, OutlinedArc arc)
    {
        ...
        colorX c = new colorX(0.9f, 1f, (ColorProfile)1);
        colorDriver.SetColors(in c);
        c = new colorX(0.1f, 1f, (ColorProfile)1);
    ...
    {
        Next("Arc");
        ArcData result = default(ArcData);
        result.arc = Current.AttachComponent<OutlinedArc>();
        result.arcLayout = Current.AttachComponent<ArcSegmentLayout>();
        if (setupButton)
        {
